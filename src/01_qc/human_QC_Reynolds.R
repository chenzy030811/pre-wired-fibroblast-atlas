...
# ---- QC thresholds (custom 5% mt, consistent with reindeer / cross-species) ----
TH_FEATURE_LOW  <- 200
...
hdr <- sprintf(
"Reynolds 2021 (E-MTAB-8142, healthy adult skin) | thresholds: nFeature %s, nCount %s, %%mt < %s%%
...
out_png <- "/Users/apple/Downloads/preview_output/PREVIEW_Reynolds_QC.png"
ggsave(out_png, plot = fig,
       width = 13, height = 6.8, dpi = 300, units = "in",
       bg = "white", device = "png", type = "cairo")

saveRDS(seu_post, file.path(base_dir, "Reynolds_postQC.rds"))
cat("Wrote:", out_png, "\n")
