...
fig <- (p1a | p1b | p1c) / (p2a | p2b | p2c) +
  plot_annotation(
    title    = "Sole-Boldo 2020 - QC (uniform 5% mito thresholds for cross-species use)",
    subtitle = hdr,
    theme = theme(
...
out_png <- "/Users/apple/Downloads/preview_output/PREVIEW_SoleBoldo_QC_custom.png"
ggsave(out_png, plot = fig,
       width = 13, height = 6.8, dpi = 300, units = "in",
       bg = "white", device = "png", type = "cairo")

saveRDS(seu_post, file.path(base_dir, "SoleBoldo_postQC_custom.rds"))
cat("Wrote:", out_png, "\n")
