...
TH_COUNT_LOW    <- 2500
TH_COUNT_HIGH   <- Inf
TH_MT           <- Inf     # paper does not filter by mitochondrial %
...
hdr <- sprintf(
"Tabula Sapiens 2022 (10x 3' v3, skin only, healthy) | paper thresholds: nFeature %s, nCount %s, %%mt < %s%%
...
fig <- (p1a | p1b | p1c) / (p2a | p2b | p2c) +
  plot_annotation(
    title    = "Tabula Sapiens 2022 human skin scRNA-seq - quality control",
    subtitle = hdr,
    theme = theme(
...
out_png <- "/Users/apple/Downloads/preview_output/PREVIEW_TabulaSapiens_QC.png"
ggsave(out_png, plot = fig,
       width = 13, height = 6.8, dpi = 300, units = "in",
       bg = "white", device = "png", type = "cairo")

saveRDS(seu_post, file.path(base_dir, "TabulaSapiens_postQC.rds"))
cat("Wrote:", out_png, "\n")
