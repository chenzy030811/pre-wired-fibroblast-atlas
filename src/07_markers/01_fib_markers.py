"""
05b — Marker gene discovery for the 12 cross-species fibroblast clusters.

Input:
  integrated.h5ad  (raw counts in .X, HVG 3000, leiden_scvi labels)

Steps:
  1. normalize_total + log1p on raw counts
  2. rank_genes_groups (Wilcoxon) per Leiden cluster
  3. Export CSV: top 30 markers per cluster + stats
  4. Plot heatmap: 12 clusters × top 5 markers
  5. Plot dotplot: 12 clusters × canonical fibroblast subtype markers
  6. Print which hero genes are present in HVG (for planning next step)

Output:
  results/fib_cluster_markers.csv
  preview_output/PREVIEW_Fib_marker_heatmap.png
  preview_output/PREVIEW_Fib_marker_dotplot.png
"""
...
# ============ palette — Nature-style pink-to-blue-grey (user-selected) ==
# Anchor codes user provided:
...
adata.obs_names_make_unique()          # fix duplicate warning
print(f"    {adata.n_obs} cells × {adata.n_vars} HVG")

print(">>> Normalising + log1p ...")
adata.layers["counts"] = adata.X.copy()  # keep raw for later
sc.pp.normalize_total(adata, target_sum=1e4)
sc.pp.log1p(adata)

# force numeric cluster ordering (0,1,2,...,11)  — fixes string-sort bug
...
sc.tl.rank_genes_groups(
    adata, "leiden_scvi",
    method="wilcoxon",
    pts=True,       # % of cells expressing (useful for interpretation)
    groups=_uniq,   # explicit numeric order
)
...
csv_path = results / "fib_cluster_markers.csv"
pd.DataFrame(rows).to_csv(csv_path, index=False)
print(f"    → {csv_path}")

# --------- heatmap: use sc.pl.matrixplot with explicit categories_order
...
sc.pl.matrixplot(
    ...
    categories_order=_uniq,          # force numeric order 0..11
    cmap=CMAP_PINK_BLUE,
    standard_scale="var",
...
# panel-mode: no suptitle (compose adds it externally)
plt.savefig(preview / "PREVIEW_Fib_marker_heatmap.png",
            dpi=400, bbox_inches="tight")
plt.close()
print(f"    → {preview}/PREVIEW_Fib_marker_heatmap.png")

# --------- dotplot: canonical fibroblast subtype markers ---------
# Curated markers used across mouse/human/reindeer skin fib papers.
canonical = {
    "Universal":    ["PDGFRA", "COL1A1", "DCN", "LUM"],
    "Papillary":    ["DPP4", "WIF1", "LRIG1", "COL18A1"],
...
# Flatten + drop duplicates, keep only those present in HVG
seen, ordered = set(), []
...
print(f"    {len(ordered)} canonical markers found in HVG")
if missing_hero:
    print(f"    ⚠️  Hero genes NOT in HVG: {missing_hero}")
    print("       → need to bring these in from merged_Fibroblast.h5ad (step 06)")

# -------- CUSTOM DOTPLOT (reference-style: rows=genes, cols=clusters,
#          dotted horizontal guides, vertical colour bar, vertical size legend) --
...
def custom_dotplot(adata, genes, groupby, cmap, out_path, title):
    ...
    # scale per gene (0..1) so each row is comparable across clusters
    lo = mean_exp.min(axis=0); hi = mean_exp.max(axis=0)
    rng = np.where(hi - lo == 0, 1, hi - lo)
    scaled = (mean_exp - lo) / rng

    # ---- figure layout: main plot + two compact legends in top-right corner ----
    fig_w = 0.32*n_g + 3.0    # narrower per-cluster column
    fig_h = 0.22*n_c + 1.3    # shorter per-gene row
    fig = plt.figure(figsize=(fig_w, fig_h))

    # main dot area — leaves room on the right for legends
    ax = fig.add_axes([0.13, 0.08, 0.63, 0.85])
    # small vertical colourbar in top-right
    axcb = fig.add_axes([0.82, 0.55, 0.018, 0.30])
    # compact vertical size legend below colourbar
    axsz = fig.add_axes([0.82, 0.15, 0.10, 0.30])

    # ---- guide grid (horizontal dotted lines, one per gene) ----
    for j in range(n_c):
    ...
    # ---- dots ----
    max_dot = 110  # dots scale down with the shrunk cells
    for i in range(n_g):
        for j in range(n_c):
    ...
    # ---- vertical size legend (5 sizes, tight spacing) ----
...
print(">>> Drawing custom reference-style dotplot ...")
custom_dotplot(
    adata, ordered, "leiden_scvi", CMAP_PINK_BLUE,
    preview / "PREVIEW_Fib_marker_dotplot.png",
    "Canonical fibroblast subtype markers × 12 clusters",
)
print(f"    → {preview}/PREVIEW_Fib_marker_dotplot.png")
