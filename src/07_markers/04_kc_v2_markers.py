"""
05e — Marker analysis on Keratinocyte v2 scVI (5-group data, winner).

Input:  scvi_out/Keratinocyte_v2_scvi/integrated.h5ad
Output:
  results/kc_v2_cluster_markers.csv
  preview_output/PREVIEW_v2_KC_marker_heatmap.png
  preview_output/PREVIEW_v2_KC_marker_dotplot.png
"""
...
df_all.to_csv(results / "kc_v2_cluster_markers.csv", index=False)
print(f"    → {results}/kc_v2_cluster_markers.csv")
...
plt.savefig(preview / "PREVIEW_v2_KC_marker_heatmap.png", dpi=400, bbox_inches="tight")
plt.close()
print(f"    → {preview}/PREVIEW_v2_KC_marker_heatmap.png")
...
print(f"    {len(ordered)} canonical KC markers present")

def custom_dotplot(adata, genes, groupby, cats_ordered, cmap, out_path):
...
print(">>> Dotplot (canonical KC markers) ...")
custom_dotplot(adata, ordered, "leiden_scvi", _uniq, CMAP_PB,
               preview / "PREVIEW_v2_KC_marker_dotplot.png")
print(f"    → {preview}/PREVIEW_v2_KC_marker_dotplot.png")
